...
class PathManager(object):
    ...
    def get_path_fillet(self, new_waypoints, pos):
        c = -5
        lam = -5
        if not np.array_equal(self.waypoints, new_waypoints):
            self.waypoints = new_waypoints
            self.waypoint_counter = 1
            self.state = 1
    ...
    def get_path_dubins(self, new_waypoints, pos):
        ...
        [L, cs, lam_s, ce, lam_e, z1, q1, z2, z3, q3] = find_dubins_parameters(w_past, chi_past, w_now, chi_now)
        r = 'r'
        q = 'q'
        c = 'c'
        lam = 'lam'
...
def find_dubins_parameters(ps, chi_s, pe, chi_e):
    dist = np.linalg.norm(ps - pe)
    e1 = np.array([[1], [0], [0]])

    assert (dist >= 3 * P.t_rad), "waypoints are too close together!"
    # assert (self.fillet_rad > minTurnRad)
    crs = ps + P.t_rad * np.matmul(rotz(np.pi/2), np.array([[cos(chi_s)], [sin(chi_s)], [0]]))
    cls = ps + P.t_rad * np.matmul(rotz(-np.pi/2), np.array([[cos(chi_s)], [sin(chi_s)], [0]]))
    cre = pe + P.t_rad * np.matmul(rotz(np.pi/2), np.array([[cos(chi_e)], [sin(chi_e)], [0]]))
    cle = pe + P.t_rad * np.matmul(rotz(-np.pi/2), np.array([[cos(chi_e)], [sin(chi_e)], [0]]))

    #compute length for case 1 rsr
    ang = atan2(cre.item(1)-crs.item(1), cre.item(0)-crs.item(0))
    L1 = np.linalg.norm(crs-cre) + P.t_rad * mod2pi(2 * np.pi + mod2pi(ang - np.pi / 2) - mod2pi(chi_s - np.pi / 2)) \
         + P.t_rad * mod2pi(2 * np.pi + mod2pi(chi_e - np.pi / 2) - mod2pi(ang - np.pi / 2))

    # Compute length for case 2 rsl
    ang = atan2(cle.item(1)-crs.item(1), cle.item(0)-crs.item(0))
    l = np.linalg.norm(cle - crs)
    ang2 = ang - np.pi / 2 + asin((2 * P.t_rad) / l)
    L2 = np.sqrt(l ** 2 - 4 * P.t_rad ** 2) + P.t_rad * mod2pi(2 * np.pi + mod2pi(ang2) - mod2pi(chi_s - np.pi / 2)) \
         + P.t_rad * mod2pi(2 * np.pi + mod2pi(ang2 + np.pi) - mod2pi(chi_e + np.pi / 2))

    # Compute length for case 3 lsr
    ang = atan2(cre.item(1)-cls.item(1), cre.item(0)-cls.item(0))
    l = np.linalg.norm(cre-cls)
    ang2 = acos((2 * P.t_rad) / l)
    L3 = np.sqrt(l ** 2 - 4 * P.t_rad ** 2) + P.t_rad * mod2pi(2 * np.pi + mod2pi(chi_s + np.pi / 2) - mod2pi(ang + ang2)) \
         + P.t_rad * mod2pi(2 * np.pi + mod2pi(chi_e - np.pi / 2) - mod2pi(ang + ang2 - np.pi))

    # Compute length for case 4 lsl
    ang = atan2(cle.item(1)-cls.item(1), cle.item(0)-cls.item(0))
    L4 = np.linalg.norm(cls-cle) + P.t_rad * mod2pi(2 * np.pi + mod2pi(chi_s + np.pi / 2) - mod2pi(ang + np.pi / 2)) \
         + P.t_rad * mod2pi(2 * np.pi + mod2pi(ang + np.pi / 2) - mod2pi(chi_e + np.pi / 2))
    ...
    if min(lengths) == L1:
        ...
        z1 = cs + P.t_rad * np.matmul(rotz(-np.pi/2), q1)
        z2 = ce + P.t_rad * np.matmul(rotz(-np.pi/2), q1)
        ...
        z1 = cs + P.t_rad * np.matmul(rotz(ang2), e1)
        z2 = ce + P.t_rad * np.matmul(rotz(ang2 + np.pi), e1)
        ...
        z1 = cs + P.t_rad * np.matmul(rotz(ang + ang2), e1)
        z2 = ce + P.t_rad * np.matmul(rotz(ang + ang2 -np.pi), e1)
